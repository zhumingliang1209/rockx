...
rockx_handle_t face_liveness2d_handle;
...
int run_face_liveness(rockx_image_t *in_image, rockx_face_liveness_t *result) {
    rockx_ret_t ret;
    ...
    // Face Recognition
    rockx_face_liveness_detect(face_liveness2d_handle, &out_img, result);
...
int main(int argc, char** argv) {
    ...
    ret = rockx_create(&face_liveness2d_handle, ROCKX_MODULE_FACE_LIVENESS, nullptr, 0);
    if (ret != ROCKX_RET_SUCCESS) {
        printf("init rockx module ROCKX_MODULE_FACE_LIVENESS error %d\n", ret);
        return -1;
    }
    ...
    rockx_face_liveness_t result;

    run_face_liveness(&input_image1, &result);

    printf("%f\t%f\n", result.fake_score, result.real_score);
    //release handle
    rockx_destroy(face_det_handle);
    rockx_destroy(face_5landmarks_handle);
    rockx_destroy(face_liveness2d_handle);
    return 0;
}
